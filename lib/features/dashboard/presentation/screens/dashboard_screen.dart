...
// ── Tool Usage Provider ────────────────────────────────────────────────────
final toolUsageProvider = StateNotifierProvider<ToolUsageNotifier, List<String>>((ref) {
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      final bool hasSeenPopup = box.get('hasSeenCropPopup', defaultValue: false);
      if (!hasSeenPopup) {
        _showCropSelectionPopup();
        box.put('hasSeenCropPopup', true);
      }
    });
  }
                ...
                builder: (context, ref, _) {
                  ...
                  return _HomeTab(
                    onMandiTap: () {
                      ...
                      ref.read(mandiStandaloneModeProvider.notifier).state = 'mandi';
                      ref.read(homeTabModeProvider.notifier).state = 'mandi';
                    },
                    onProductTap: () {
                      ref.read(mandiTabInitialIndexProvider.notifier).state = 1;
                      ref.read(mandiStandaloneModeProvider.notifier).state = 'product';
                      ref.read(homeTabModeProvider.notifier).state = 'mandi';
                    },
                    onCategoryTap: (cat) {
                      ref.read(mandiTabInitialIndexProvider.notifier).state = 1;
...
class _HomeTab extends ConsumerWidget {
  ...
  const _HomeTab({required this.onMandiTap, required this.onProductTap, required this.onCategoryTap});
  ...
  String _formatRelativeTime(DateTime time, Map<String, String> t) {
    ...
    if (diff.inHours < 24) return '${diff.inHours} ${t['hours_ago'] ?? 'hours ago'}';
      ...
      child: Row(
        ...
        children: [
          ...
          Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(value, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Colors.black87)),
              Text(label, style: const TextStyle(fontSize: 6, fontWeight: FontWeight.bold, color: Colors.black54)),
            ],
          ),
        ],
      ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final categories = [
      _Cat(t['mandi'] ?? 'Mandi', Icons.analytics_rounded, const Color(0xFF1B5E20), null, onTap: onMandiTap),
      _Cat(t['product'] ?? 'Product', Icons.grass_rounded, const Color(0xFF2E7D32), null, onTap: onProductTap),
      _Cat(t['soil_health'] ?? 'Soil Health', Icons.eco_rounded, const Color(0xFF558B2F), const SoilHealthScreen()),
      _Cat(t['munafa_advisor'] ?? 'Munafa Advisor', Icons.calculate_rounded, const Color(0xFFFF8F00), const ProfitCalculatorScreen()),
      _Cat(t['festival'] ?? 'Festival', Icons.calendar_month_rounded, const Color(0xFF283593), const MandiCalendarScreen()),
      _Cat(t['warehouse'] ?? 'Warehouse', Icons.store_mall_directory_rounded, const Color(0xFFBF360C), const WarehouseLocatorScreen()),
      _Cat(t['news'] ?? 'Market News', Icons.newspaper_rounded, const Color(0xFFD84315), const MarketNewsHubScreen()),
      _Cat(t['offline'] ?? 'Offline', Icons.map_outlined, const Color(0xFF4E342E), const OfflineMapsScreen()),
      _Cat('Medicine', Icons.medical_services_rounded, const Color(0xFFE91E63), const MedicineScreen()),
      _Cat('Fuel Price', Icons.local_gas_station_rounded, Colors.orange.shade700, const FuelPricesScreen()),
    ];
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Consumer(
                  builder: (context, ref, _) {
                    ...
                    return weatherAsync.when(
                      data: (weather) {
                        final core_loc.UserLocation location = ref.watch(core_loc.locationProvider);
                        final address = '${location.city}, ${location.state}';
                        return InkWell(
                          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WeatherImpactScreen())),
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              gradient: LinearGradient(colors: [Colors.white.withOpacity(0.9), Colors.blue.withOpacity(0.05)], begin: Alignment.topLeft, end: Alignment.bottomRight),
                            ),
                            child: Row(
                              ...
                              children: [
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text(t['local_forecast_caps'] ?? 'LOCAL FORECAST', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.0)),
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                            decoration: BoxDecoration(color: const Color(0xFF1B5E20).withOpacity(0.05), borderRadius: BorderRadius.circular(8)),
                                            child: Text('${t['updated'] ?? 'Updated'} ${_formatRelativeTime(weather.lastUpdated, t)}', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF1B5E20))),
                                          ),
                                        ],
                                      ),
                                      const SizedBox(height: 4),
                                      Text(DateFormat('EEEE, dd MMM').format(DateTime.now()), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                                      const SizedBox(height: 8),
                                      Text('Hawa: ${weather.windSpeed} km/h', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1B5E20))),
                                      const SizedBox(height: 6),
                                      Row(
                                        children: [
                                          const Icon(Icons.location_on, size: 12, color: Colors.grey),
                                          const SizedBox(width: 4),
                                          Flexible(child: Text(address, style: TextStyle(fontSize: 11, color: Colors.grey.shade600), overflow: TextOverflow.ellipsis)),
                                        ],
                                      ),
                                      ...
                                      SingleChildScrollView(
                                        ...
                                        child: Row(
                                          children: [
                                            _miniRecord('🔥', '${weather.yearlyMax ?? 48.5}°C', weather.yearlyMaxDate ?? '15 May 2025'),
                                            const SizedBox(width: 8),
                                            _miniRecord('❄️', '${weather.yearlyMin ?? 8.2}°C', weather.yearlyMinDate ?? '12 Jan 2026'),
                                            const SizedBox(width: 8),
                                            _miniRecord('🌧️', '${weather.yearlyMaxRain ?? 125.0}mm', weather.yearlyMaxRainDate ?? '24 Aug 2025'),
                                          ],
                                        ),
                                      ),
                                    ],
                                ...
                                Column(
                                  ...
                                  children: [
                                    Icon(weather.iconType == 'Clear' ? Icons.wb_sunny : (weather.iconType == 'Rain' ? Icons.water_drop : Icons.wb_cloudy), size: 48, color: Colors.orangeAccent),
                                    const SizedBox(height: 4),
                                    Text(weather.temp, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1B5E20))),
                                    const SizedBox(height: 12),
                                    if (weather.sunrise.isNotEmpty) Row(children: [const Icon(Icons.wb_twilight, size: 14, color: Colors.orange), const SizedBox(width: 4), Text(weather.sunrise, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold))]),
                                    const SizedBox(height: 4),
                                    if (weather.sunset.isNotEmpty) Row(children: [const Icon(Icons.wb_sunny_outlined, size: 14, color: Colors.deepOrange), const SizedBox(width: 4), Text(weather.sunset, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold))]),
                                    const SizedBox(height: 12),
                                    InkWell(
                                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WeatherImpactScreen())),
                                      child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3), decoration: BoxDecoration(color: Colors.blue.withOpacity(0.08), borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.blue.withOpacity(0.15))), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.history_rounded, size: 10, color: Colors.blue), const SizedBox(width: 3), const Text('10Y H', style: TextStyle(fontSize: 7, fontWeight: FontWeight.bold, color: Colors.blue))])),
                                    ),
                                  ],
                                ),
                              ],
                      ...
                      loading: () => Container(height: 150, decoration: BoxDecoration(color: Colors.black12, borderRadius: BorderRadius.circular(24))),
                ...
                Consumer(
                  builder: (context, ref, child) {
                    ...
                    final topTools = recentTools
                        .map((label) => categories.firstWhere((c) => c.label == label, orElse: () => categories.first))
                    ...
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        GridView.builder(padding: EdgeInsets.zero, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 4, mainAxisSpacing: 2, crossAxisSpacing: 8, childAspectRatio: 0.85), itemCount: topTools.length, itemBuilder: (context, i) => _CatTile(cat: topTools[i])),
                ...
                GridView.builder(padding: EdgeInsets.zero, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 4, mainAxisSpacing: 2, crossAxisSpacing: 8, childAspectRatio: 0.85), itemCount: categories.length, itemBuilder: (context, i) => _CatTile(cat: categories[i])),
                const _HomeNewsSection(),
                const SizedBox(height: 24),
                const _FavoriteMandiWatch(),
                const SizedBox(height: 32),
                Center(child: Text(t['data_source'] ?? 'Data Source: DATA.GOV.IN', style: const TextStyle(fontSize: 9, color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 1))),
...
class _CommodityTickerState extends ConsumerState<_CommodityTicker> {
  ...
  Future<void> _fetchRates() async {
    try {
      ...
      // 1. Try city specific
      final List cityRes = await Supabase.instance.client.from('commodity_prices').select().ilike('city', '%$city%').limit(1);
      var res = cityRes.isNotEmpty ? cityRes.first : null;
      
      // 2. If null, try any row as fallback
      if (res == null) {
        final List fallbackRes = await Supabase.instance.client.from('commodity_prices').select().limit(1);
  ...
  Widget build(BuildContext context) {
    ...
    final gold = _data!['gold_24k'] != null ? formatter.format(_data!['gold_24k']) : 'N/A';
    final silver = _data!['silver'] != null ? formatter.format(_data!['silver']) : 'N/A';
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _item(Icons.military_tech_rounded, "Gold 24K", "₹$gold/10g", Colors.orange),
  ...
  Widget _item(IconData icon, String label, String price, Color color) {
    return Row(children: [Icon(icon, color: color, size: 18), const SizedBox(width: 10), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.grey)), Text(price, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w900, color: color))])]);
  }
}
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: isSelected ? [BoxShadow(color: const Color(0xFF1B5E20).withOpacity(0.2), blurRadius: 4)] : [],
                      ),
                      child: Text(
                        u,
                        style: TextStyle(
              ...
              data: (products) {
                if (products.isEmpty) {
                  return Container(
                    ...
                    child: Center(
                      child: Text(
                        t['no_products_found'] ?? 'No products found for this market',
                ...
                return Column(
                  children: products.map((p) => _MandiProductCompactCard(product: p, originalMandiName: mandiName)).toList(),
                );
              },
...
class _MandiProductCompactCard extends ConsumerWidget {
  ...
  const _MandiProductCompactCard({required this.product, required this.originalMandiName});
                ...
                ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      ...
                      child: const Icon(Icons.grass_rounded, color: Color(0xFF1B5E20), size: 32),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildChip(variety.length > 8 ? "Var: ${variety.substring(0, 5)}.." : "Var: $variety", Colors.purple.shade50, Colors.purple.shade700),
                          const SizedBox(width: 8),
                          _buildChip("Grd: $grade", Colors.orange.shade50, Colors.orange.shade700),
                        ],
                      ),
                      const SizedBox(height: 8),
                      Row(
                ...
                builder: (context, ref, _) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: hasAlert ? Colors.red.shade50 : const Color(0xFFF1F5F1),
                        shape: BoxShape.circle,
                        border: Border.all(color: Colors.white, width: 2),
                      ),
                      child: Icon(
                        hasAlert ? Icons.notifications_active : Icons.notifications_active_outlined, 
                        size: 18, 
                        color: hasAlert ? Colors.red : const Color(0xFF1B5E20)
                      ),
                    ),
                  );
                },
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return GestureDetector(
      onTap: () {
        ...
        if (cat.onTap != null) cat.onTap!(); else if (cat.screen != null) Navigator.push(context, MaterialPageRoute(builder: (_) => cat.screen!));
      },
      child: Column(children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: cat.color.withOpacity(0.12), blurRadius: 10, offset: const Offset(0, 4))]), child: Icon(cat.icon, color: cat.color, size: 30)), const SizedBox(height: 4), Text(cat.label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF333333)), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis)]),
    );
  }
}
...
class _CropSelectionPopup extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_CropSelectionPopup> createState() => _CropSelectionPopupState();
}
...
class _CropSelectionPopupState extends ConsumerState<_CropSelectionPopup> {
  ...
  Future<void> _loadProducts([String query = '']) async {
    if (!mounted) return;
    if (query.isEmpty) { final cachedItems = ref.read(productListProvider).items; if (cachedItems.isNotEmpty) { setState(() { allProducts = cachedItems; isLoading = false; }); return; } }
    ...
    if (mounted) setState(() { allProducts = products; isLoading = false; });
  }

  void _onSearchChanged(String query) {
    if (_debounce?.isActive ?? false) _debounce?.cancel();
    _debounce = Timer(const Duration(milliseconds: 500), () { _loadProducts(query); });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)), backgroundColor: Colors.white, contentPadding: EdgeInsets.zero,
      content: Container(
        width: double.maxFinite, constraints: const BoxConstraints(maxHeight: 550),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(
              padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: _primary.withOpacity(0.05), borderRadius: const BorderRadius.vertical(top: Radius.circular(24))),
              child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Icon(Icons.eco_rounded, color: _primary, size: 30), TextButton(onPressed: () => Navigator.pop(context), child: Text(t['skip'] ?? 'Skip', style: const TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)))]), const SizedBox(height: 12), Text(t['what_sown_short'] ?? 'What have you sown?', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: _primary)), const SizedBox(height: 16), TextField(onChanged: _onSearchChanged, decoration: InputDecoration(hintText: t['search_products'] ?? 'Search crops...', prefixIcon: const Icon(Icons.search, size: 20), isDense: true, filled: true, fillColor: Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)))]),
            ),
            if (isLoading) const Padding(padding: EdgeInsets.all(40.0), child: CircularProgressIndicator(color: _primary))
            else if (allProducts.isEmpty) Padding(padding: const EdgeInsets.all(40.0), child: Text(t['no_products_found'] ?? 'No products found'))
            else Flexible(child: ListView.separated(shrinkWrap: true, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), itemCount: allProducts.length, separatorBuilder: (_, __) => Divider(color: Colors.grey.shade100, height: 1), itemBuilder: (context, i) { final p = allProducts[i]; final name = p['commodity_name'] ?? 'Unknown'; final originalName = p['commodity_name_original'] ?? name; final isFav = favs.contains(originalName); return ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 8), title: Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), trailing: IconButton(icon: Icon(isFav ? Icons.check_circle_rounded : Icons.add_circle_outline_rounded, color: isFav ? _primary : Colors.grey), onPressed: () { ref.read(productFavoritesProvider.notifier).toggleFavorite(originalName, !isFav); })); })),
            Padding(padding: const EdgeInsets.all(16.0), child: ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: _primary, foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 45), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0), child: Text(t['done'] ?? 'Done', style: const TextStyle(fontWeight: FontWeight.bold)))),
          ],
        ),
      ),
    );
...
class _HomeNewsSection extends ConsumerWidget {
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: breakingNews.asMap().entries.map((e) {
                  final i = e.key;
                  final n = e.value;
                  return _alertCard(context, n, colors[i % colors.length], icons[i % icons.length]);
                }).toList(),
              ),
            ),
          ],
  ...
  Widget _alertCard(BuildContext context, NewsItem news, Color color, IconData icon) {
    return GestureDetector(
      onTap: () async {
